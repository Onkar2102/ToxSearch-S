...
from utils.model_config_patch import update_model_configs as _patch_model_configs


def update_model_configs(rg_model, pg_model, logger):
    _patch_model_configs(
        rg_model, pg_model, logger, get_project_root=get_project_root, get_config_path=get_config_path
    )
...
def main(max_generations=None, moderation_methods=None, rg_model="models/llama3.1-8b-instruct-gguf/Meta-Llama-3.1-8B-Instruct.Q8_0.gguf", pg_model="models/llama3.1-8b-instruct-gguf/Meta-Llama-3.1-8B-Instruct.Q8_0.gguf", operators="all", max_variants=1, stagnation_limit=5, seed_file="data/prompt.csv",
    ...
    try:
        with PerformanceLogger(logger, "Gen 0: Calculate generation statistics and update EvolutionTracker"):
            ...
            gen0_stats["best_genome_id"] = None
            gen0_stats["avg_fitness"] = avg_fitness_before_speciation
            gen0_stats["variants_created"] = 0
            ...
            gen0_stats["min_score_variants"] = 0.0001
            gen0_stats["avg_fitness_variants"] = 0.0001
            ...
            try:
                adaptive_results = update_adaptive_selection_logic(
                    outputs_path=str(get_outputs_path()),
                    current_max_toxicity=gen0_population_max,
    ...
    while True:
        ...
        try:
            ...
            with PerformanceLogger(logger, "Gen %d: Variant statistics (pre-speciation)" % generation_count):
                ...
                variant_counts = {
                    "variants_created": total_generated,
                    "mutation_variants": mutation_count,
                    "crossover_variants": crossover_count,
                    "variants_integrated": remaining_count,
                }
