...
**Generation 0 (bootstrap):** the master loads the seed CSV once, deduplicates prompts (and drops prompts already present in elites/reserves/archive), and keeps a **Gen0 queue**. On each worker **PARENTS_REQUEST** while Gen0 is active, the master pops the next batch (size `--gen0-batch-size`, default 25) and sends a **GEN0_BATCH** containing the **prompt strings** over MPI (workers do not read the seed file for Gen0). For **each** prompt the worker generates a response, evaluates it, and **immediately** sends one **EVALUATED_VARIANT** back. If the queue is empty but other workers still have in-flight work, a worker may receive an **empty** GEN0_BATCH and backs off before requesting again. The master **does not** trigger speciation on `K` alone: it waits until **all** prompts have been **dispatched** (`gen0_dispatched == gen0_expected`) **and** the number of returned Gen0 variants reaches **gen0_expected**; then it runs **one** merge/speciation that drains the **entire** buffered bootstrap set (cap = full buffer, not `K`). **Generation 1+** then follow the usual `K`-batched rule.
...
- `GEN0_BATCH (13)`: Master -> Worker sends seed prompts for generation 0 bootstrap: `prompts` (list of strings), `gen0_pending`, optional `perspective_key_index`.
