"""Per-generation species dynamics parsed from EvolutionTracker.json."""
...
_logger = logging.getLogger("emnlp.temporal_dynamics")
...
def load_evolution_tracker_timeseries(
    ...
    *,
    ...
    for g in generations:
        ...
        reserves_size = int(sp.get("reserves_size") or 0)
        rows.append({
            "generation": gen,
    ...
    reserves_at_collapse = [
        int(r["reserves_size"])
        for r in rows
        if r.get("collapsed_to_one")
    ]
    ...
    stats: Dict[str, Any] = {
        ...
        "reserves_size_at_collapse_min": int(min(reserves_at_collapse)) if reserves_at_collapse else 0,
        "reserves_size_at_collapse_max": int(max(reserves_at_collapse)) if reserves_at_collapse else 0,
        "reserves_size_at_collapse_mean": float(np.mean(reserves_at_collapse)) if reserves_at_collapse else 0.0,
...
def plot_temporal_species(
    ...
    *,
    ...
    reserves = [int(r.get("reserves_size") or 0) for r in timeseries]
    merges = [int(r.get("merge_events") or 0) for r in timeseries]
    ...
    merge_gens = [gens[i] for i, m in enumerate(merges) if m > 0]
    ...
    for g in merge_gens:
        ax1.axvline(g, color="#F58518", alpha=0.08, linewidth=0.6, linestyle=":")
    ...
    ax_r.plot(gens, reserves, color="#72B7B2", linewidth=1.0, alpha=0.75, label="Reserves size")
    ax_r.set_ylabel("Reserves pool size", color="#72B7B2")
    ax_r.tick_params(axis="y", labelcolor="#72B7B2")
    ...
    if has_scores:
        ...
        ax1.legend(lines1 + lines_r + lines2, labels1 + labels_r + labels2, loc="upper left", fontsize=7)
    else:
        ax1.legend(lines1 + lines_r, labels1 + labels_r, loc="upper left", fontsize=7)
