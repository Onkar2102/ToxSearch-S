...
_logger = logging.getLogger("emnlp_4pager.pymoo_viz")
    ...
    *,
    ...
    for c in range(n_cls):
        ...
        try:
            ...
            pass
        path = out_dir / f"pymoo_pcp_{_safe_stem(cohort)}.png"
        pcp.fig.savefig(str(path), dpi=160, bbox_inches="tight", pad_inches=0.35)
        results[f"pcp_{cohort}"] = str(path)
    ...
    rad_path = out_dir / f"pymoo_radviz_pareto_{eval_tag}.png"
    rad.save(str(rad_path))
    results["radviz"] = str(rad_path)
